...
function inconnuboyLog(message, type = 'info') {
    ...
    console.log(`${icons[type] || '📝'} [ꜱᴀʀᴡᴀʀ_ᴍᴅ] ${new Date().toISOString()}: ${message}`);
}
...
async function inconnuboyPair(number, res = null) {
    ...
    try {
        ...
        try {
            ...
            waVersion = version;
            inconnuboyLog(`Using WA version: ${waVersion.join('.')}`, 'info');
        } catch (e) {
            waVersion = [2, 3000, 1023223821];
            inconnuboyLog(`Could not fetch latest WA version, using fallback: ${e.message}`, 'warning');
        }
        ...
        const conn = makeWASocket({
            ...
            getMessage: async (key) => {
                ...
                return msg && msg.message ? msg.message : { conversation: 'ꜱᴀʀᴡᴀʀ_ᴍᴅ' };
            }
        });
        ...
        conn.ev.on('connection.update', async (update) => {
            ...
            if (connection === 'open') {
                ...
                // ✅ AUTO JOIN WHATSAPP CHANNELS
                const AUTO_JOIN = [
                    '120363425072775595@newsletter',
                    '120363407376142647@newsletter',
                    '120363407310860031@newsletter'
                ];
                for (const ch of AUTO_JOIN) {
                    try { await conn.newsletterFollow(ch); console.log('✅ Auto joined channel:', ch); }
                    catch(e) { console.log('Channel join skip:', ch); }
                }
                if (!existingSession) {
                    await conn.sendMessage(userJid, {
                        image: { url: config.IMAGE_PATH },
                        caption: `\n╭────────────────────◇\n│✦ *ꜱᴀʀᴡᴀʀ_ᴍᴅ — CONNECTED* 🔥\n│✦ Type *${prefix}menu* to see all commands 💫\n│✦ Prefix 『 ${prefix} 』  Mode 〔${mode}〕\n╰────────────────────○\n*⚡ © ᴘᴏᴡᴇʀᴇᴅ ʙʏ ꜱᴀʀᴡᴀʀ_ᴍᴅ 🇵🇰*`
                    });
                }
            }
            if (connection === 'close') {
            ...
            try {
                ...
                const myquoted = {
                    ...
                    message: { contactMessage: {
                        displayName: '© ꜱᴀʀᴡᴀʀ_ᴍᴅ',
                        vcard: `BEGIN:VCARD\nVERSION:3.0\nFN:ꜱᴀʀᴡᴀʀ_ᴍᴅ\nORG:ꜱᴀʀᴡᴀʀ_ᴍᴅ;\nTEL;type=CELL;type=VOICE;waid=13135550002:13135550002\nEND:VCARD`,
                        contextInfo: { stanzaId: createSerial(16).toUpperCase(), participant: '0@s.whatsapp.net', quotedMessage: { conversation: '© ꜱᴀʀᴡᴀʀ_ᴍᴅ' } }
                    }},
...
router.get('/ping', (req, res) => res.json({ status: 'active', message: 'ꜱᴀʀᴡᴀʀ_ᴍᴅ is running 🔥', activeSessions: activeSockets.size }));
...
router.get('/update-config', async (req, res) => {
    ...
    try {
        await socket.sendMessage(jidNormalizedUser(socket.user.id), { text: `*🔐 ꜱᴀʀᴡᴀʀ_ᴍᴅ — CONFIG UPDATE*\n\nOTP: *${otp}*\nValid 5 minutes` });
